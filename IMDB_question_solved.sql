...
SELECT Count(*) AS director_mapping_total_rows
FROM   director_mapping;

SELECT Count(*) AS role_mapping_total_rows
FROM   role_mapping; 

-- Used SQL Formatter & Beautifier to present clean codes, and they DO look good indeed!
...
-- First, let's count total number of movies released each year
SELECT year,
       Count(year) AS number_of_movies_each_year
FROM   movie
GROUP  BY year; 

-- One clear learning is that no. of movies have reduced with each passing year

-- Now, let's figure out the monthwise trend. Each month will be represented by month number (month_num)
...
-- As we can see, a total of 1,059 movies were made in the year 2019.
...
SELECT     genre,
           Count(m.id) AS number_of_movies_in_genre
FROM       movie       AS m
INNER JOIN genre       AS g
where      g.movie_id = m.id
GROUP BY   genre
ORDER BY   number_of_movies_in_genre DESC LIMIT 1 ;

-- As we can see, RSVP made most movies in Drama genre.
...
-- A total of 3,289 movies were made in one single genre.
...
WITH ActorRatings AS (
    ...
    WHERE 
        m.country = 'India'
        AND rm.category = 'Actor'  -- Ensuring only actors are considered
    GROUP BY 
        n.name
    HAVING 
        COUNT(m.id) >= 5
...
WITH thriller_movies
     AS (SELECT DISTINCT title             AS movie_name,
                         Avg(r.avg_rating) AS rating
         FROM   movie AS m
                INNER JOIN ratings AS r
                        ON r.movie_id = m.id
...
SELECT movie_name,
       CASE
         ...
         WHEN rating BETWEEN 7 AND 8 THEN 'Hit'
         WHEN rating BETWEEN 5 AND 7 THEN 'One-time-watch'
         ELSE 'Flop'
